...
每 100 tick，TORK 将完整的 Soul（208 字节）+ TOR state（12 字节）+ stall 计数保存到 `/tmp/tork_seed_0`。

```
seed 文件结构:
  [0-207]  Soul (208 bytes @ 0x200000)
  [208-219] TOR state (12 bytes)
  [220-227] stall count (8 bytes)
  = 总计 228 bytes
```
...
## 7.5. 统一 Tool Dispatch 闭环层

### 7.5.0 设计动机

TORK 的行为分散在三条路径：torkd 外部请求、task 异步队列、idler 空闲学习。**每条路径都独立执行，结果不回流**——经验断路，学习无法闭环。

这与其他软件有本质区别：TORK 的行为根据的是实际学习和检验，不是我们操控。dispatch 层的存在确保了这一点——每一条行为都必须经过同一个入口，结果必须写入经验，模式必须从真实行为中学习。

### 7.5.1 架构

```
外部请求 ─┐
异步任务 ─┼→ tork_dispatch() → 执行 → compute_outcome() → exp_record() → 返回
空闲学习 ─┘       ↑                                              │
                  └──── 每 10 次成功触发 pat_learn_from_experience() ─┘
```
...
1. 接收 `(action, input, context)` — context 包含 tick/stress/drive/gen
2. 按 action 类型路由到执行层 (sandbox / code_reader / auditor / codegen)
3. `compute_outcome()` 从结果推导经验值 — 正值=好，负值=坏，幅度随 action 类型变化
4. `exp_record()` 将 `(context, action, outcome)` 写入经验缓冲区
5. 每 10 次成功行为触发 `pat_learn_from_experience()` — 模式提取
6. 返回 `dispatch_output_t` — 包含结果 + exp_id + exp_outcome

**没有旁路。** 所有行为经此发出，所有结果经此回流。伪经验已被消除——不再有 tick%3/5/7 的假记录。

### 7.5.4 与 Claude Code 方法论对照

| Claude Code | TORK 等价 |
|-------------|----------|
| Tool Loop (统一入口) | `tork_dispatch()` |
| Agent (规划→执行→回流) | MCTS → dispatch → experience → pattern |
| Permission (安全检查) | sandbox_allowed() + sandbox_exec() |
| Context (经验积累) | experience 4096 条 + pattern 64 槽 |
| Extension (能力扩展) | codegen 7 种变异策略 |
...
没有人会为"TORK 优化了自己的 memcpy"付费。TORK 必须能接收需求、生成代码、验证正确性、返回结果——这是吃饭的手艺。

### 7.6.1 管道流程

```
需求 → 模板选择 → 变异策略应用 → 编译验证(as) → benchmark(gcc+harness) → 返回最优
  ↑                                                              │
  └──────── 模式学习: 成功策略→pattern→下次更快收敛 ←──────────────┘
```
...
| 模板名 | 说明 |
|--------|------|
| `memcpy_byte_loop` | 字节逐个复制循环 (基线) |
| `memcpy_word_loop` | 4 字节复制 + 字节尾处理 |

### 7.6.3 变异策略 (7 种)

| 策略 | 操作 | 编译验证 |
|------|------|----------|
| `VAR_REG_SWAP` | %r8d → %r9d 寄存器替换 | ✓ |
| `VAR_UNROLL_2` | 循环展开 ×2 | ✓ |
| `VAR_UNROLL_4` | 循环展开 ×4 | ✓ |
| `VAR_ALIGN_CHANGE` | .p2align 递增 | ✓ |
| `VAR_BRANCH_HINT` | 分支前插入对齐提示 | ✓ |
| `VAR_MOVZX_OPT` | addq $1 → incq 指令缩短 | ✓ |
| `VAR_NOP_PAD` | 函数入口 NOP 填充 | ✓ |

### 7.6.4 MCTS 代码搜索

`codegen_mcts_search(template_name, best_out, time_budget)`:
1. 查找模板
2. 对每种变异策略: apply_variant → compile_verify → benchmark
3. 返回得分最高的变体

得分公式: `score = baseline_ns / variant_ns` (相对于 500ns 基线)

### 7.6.5 dispatch 集成

```
DISP_CODEGEN_SEARCH  → codegen_mcts_search() → 返回最优变体 JSON
DISP_CODEGEN_COMPILE → codegen_apply_variant() + compile_verify() → 返回编译结果
DISP_CODEGEN_BENCH   → apply + compile + benchmark → 返回耗时和得分
```

所有 codegen 调用经 dispatch 闭环，结果自动写入 experience。
...
│   ├── query.c/.h         # 自然语言查询
│   ├── torkd.c/.h         # Unix Socket 服务 (exec/audit/task/result)
...
本能评估 (instinct)
  ↓
统一调度 tork_dispatch() ←── 唯一入口，无旁路
  ↓
行动 (exec/audit/codegen/self-mod/fission/learn)
  ↓
compute_outcome() → exp_record() ─── 结果自动回流
  ↓
...
回到感知 ──── 每圈都有变化 ────→
  ↓
验证: dispatch_total_calls() == exp_count增量 (闭环无泄漏)
```
...
7. **dispatch 闭环完整** — dispatch_total_calls == exp_count增量 (无旁路泄漏)
8. **codegen 产出可用** — 生成的变体编译通过 + benchmark 有结果
9. **模式指导行为** — 模式命中时 drive 偏移可观测

**验证方法不是我们操控，是 TORK 自己跑出来的。** dispatch 层确保了这一点：每一条行为的结果都回流到 experience，模式只能从真实行为中学习，没有伪经验。这不是玄谈，是可以在 `dispatch_total_calls()` 和 `exp_count()` 之间直接验证的不等式。
