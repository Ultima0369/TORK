...
缓冲区新增 `elite_threshold` 字段（int16_t），每 32 次写入重新计算 top 10% outcome 阈值。覆写时跳过 outcome >= elite_threshold 的槽位，保护最佳经验不被新记录冲掉。
...
矩阵乘法跳过零权重（`if (w) sum += ...`），三值网络 80%+ 权重为零，稀疏化减少无效计算。

### 8.2 TLN 定向变异

观察超时后 `tln_directed_mutate()` 确保最小连通性：
- 每个输出神经元至少一个非零 w_ho 权重
- 每个隐藏神经元至少一个非零 w_ih 权重
- 对角线至少 4 个自反馈（w_hh[i][i] = +1）
...
全悬置（所有输出=0）时进入观察模式：
- 暂停代码变异（不动手）
- 每 20 tick 微调一个 w_ih 权重向 +1 偏移（蓄积倾向）
- 每 100 tick 记录快照
- 超时后触发定向变异

### 8.4 好奇心正反馈抑制

`self_tune.c` 跟踪好奇心权重连续递增次数：
- < 3 次: delta = 0.02（正常增长）
- 3-4 次: delta = 0.005（阻尼）
- >= 5 次: delta = -0.01（反转，强制收敛）
